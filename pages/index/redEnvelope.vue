<template>
	<view class="page">
		...
		<u-popup  v-model="show" :mask="mask" :mode="mode" border-radius="0">
			<view class="popDetail"> 
				<view class="popTitle text-center relative">
					...
					class="absolute-middle" @click="show = false"></image>
				</view>
				<view class="num text-center">{{money |number(4)}} {{coin}}</view>
				<view class="li flex-between align-center padding-row30">
					<view class="key">付款币种</view>
					<view class="val">{{coin}}</view>
				</view>
				<view class="li flex-between align-center padding-row30">
					<view class="key">收款方</view>
					<view class="val">红包</view>
				</view>
				<view class="li flex-between align-center padding-row30">
					<view class="key">订单信息</view>
					<view class="val">灰度红包</view>
				</view>
				<view class="li flex-between align-center padding-row30">
					<view class="key">订单金额</view>
					<view class="val">≈{{rmb |number(4)}}CNY</view>
				</view>
				<view class="btn margin120-0 text-center" @click="payment()">立即付款</view>
			</view>
		</u-popup>
		...
		<!-- 红包包好弹框 -->
		<u-popup  v-model="showRed" :mask="mask" :mode="mode2" border-radius="0">
			<view class="popRed">
				<view class="dot margin0"></view>
				<view class="name text-center">{{ name }}</view>
				<view class="msg text-center">红包已包好</view>
				<view class="send text-center margin120-0" @click="send()">发送红包</view>
			</view>
		</u-popup>
	...
	export default {
        data() {
            return {
				...
				img1:'../../static/images/app/niushi.png',
				img2:'../../static/images/app/money.png',
				text:'',
				name:'',
				url:webUrl.webUrl,
				isTrade:false,
        ...
        onShow() {
			this.name = uni.getStorageSync('hdUserInfo').address;
        ...
        methods: {
			...
			getList(){
				this.$Ajax("/rate/checkCoin",{},(res)=>{
					...
					this.rate = res.data[0].exchangeRate;  //与CNY汇率
			...
			selseimg(e) { //绘制海报
				...
				setTimeout(function() { //定时器，海报方法不能立即执行，等页面渲染完成
					...
					context.drawImage(QRCODE, 50, 376, 60, 60) //二维码图片
					context.drawImage(that.img1, 110, 220, 60, 60)
					context.drawImage(that.img2, 130, 340, 20, 25)
					
					context.setFillStyle('#fff')
					context.setFontSize(10)
					context.fillText('打开红包页面可以用', 128, 387)
					context.fill()
			
					context.setFillStyle('#fff')
					context.setFontSize(10)
					context.fillText('打折买币就挖矿', 128, 402)
					context.fill()
					
					context.setFillStyle('#fff')
					context.setFontSize(14)
					context.fillText('一币一别墅', 105, 292)
					context.fill()
					
					context.setFillStyle('#FECB95')
					context.setFontSize(10)
					context.fillText(that.text, 105, 312)
					context.fill()
					...
					context.setFillStyle('#fff')
					context.setFontSize(10)
					context.fillText('长按保存到相册分享', 128, 432)
					context.fill()
