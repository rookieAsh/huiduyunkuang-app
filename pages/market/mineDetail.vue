<template>
	<view class="page">
		...
		<view class="pic margin0">
			<image :src="obj.image" mode=""></image>
		</view>
		...
		<view class="infoBox margin30-0 padding20">
			...
			<view class="td flex">
				<view class="tdLi">{{obj.hashrate}}{{ obj.cmc_subarea == 'BTC' ? 'T' : 'M'}}H/S</view>
				<view class="tdLi">{{obj.power_diss}}KW/台</view>
			...
			<view class="td flex">
				<view class="tdLi">{{obj.earnings}}{{ obj.cmc_subarea}}/天</view>
				<view class="tdLi">{{obj.administrative_fee}}%</view>
				<view class="tdLi">{{obj.Back_tothe}}天</view>
			</view>
		</view>
		...
		<view style="width: 100%;padding: 30rpx" v-if="ver==0">
		...
		<view class="cnt margin0" v-else>
			...
			<view class="cell_td flex">
				<view class="li">支持算法</view>
				<view class="li">{{mineConfig.algorithm}}</view>
			</view>
			<view class="cell_td flex">
				<view class="li">墙上功耗</view>
				<view class="li">{{mineConfig.power_diss}}</view>
			</view>
		<!-- 	<view class="cell_td flex">
        ...
        methods: {
			...
			getDetail(){
				this.$Ajax("/MartianOrePool/selectMillAllById",{
					...
					this.obj = res.data[0];
					console.log("this.obj.image",this.obj.image)
					// var str = this.obj.end_time;
			...
			giLink(){
				this.$Ajax('/register/checkJymimaZt',{
					...
					if(res.data==0){
						...
						uni.navigateTo({
							url:'./mineBuy?productId=' + this.productId + '&quantity=' +this.quantity
						})
					}
				})
